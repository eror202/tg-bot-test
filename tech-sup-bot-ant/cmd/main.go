...
func main() {
	...
	for update := range updates {
		...
		if update.Message != nil {
			...
			case "Обратная связь":
				reply := "Коллеги, нет предела совершенству! \n\nДанный раздел предназначен для обратной связи. Наша команда всегда рада вашим идеям и предложениям по продукту. Спасибо, что помогаете нам расти и становиться лучше! " +
			...
			case "CallBacks":
				msg := tgbotapi.NewMessage(update.CallbackQuery.Message.Chat.ID, tg_bot_for_ts.CallBack)
				msg.ReplyMarkup = FAQKeyBoard
				tgUtil.SendBotMessage(msg, bot)
